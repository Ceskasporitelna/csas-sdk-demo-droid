 ...
 * @since 09/06/16.
 */
public class BuildingsListPage0Test extends TestCase {

    @Override
    public void run(TestCallback callback) {
        mTestCallback = callback;

        mNetbankingClient.getContractsResource().getBuildingsResource().list(new PaginatedParameters(new Pagination(0, 1)), new CallbackWebApi<BuildingsListResponse>() {
            @Override
