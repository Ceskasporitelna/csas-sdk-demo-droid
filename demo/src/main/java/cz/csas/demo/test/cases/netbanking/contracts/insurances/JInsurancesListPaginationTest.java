 ...
 * @since 09/06/16.
 */
public class JInsurancesListPaginationTest extends TestCase {
        ...
        ((JudgeRestService) mJudgeClient.getService()).nextCase(X_JUDGE_CASE, mXJudgeSessionHeader, new Callback<Response>() {
            ...
            public void success(Response response, Response response2) {
                ...
                for (int i = 0; i < pages.length; i++) {
                    ...
                    if (i == 2)
                        flag = true;
                    mNetbankingJudgeClient.getContractsResource().getInsurancesResource().list(new PaginatedParameters(new Pagination(page, 1)),
                                    ...
                                    switch (page) {
                                        case 0:
                                            if (!assertEquals(Long.valueOf(0), insurancesListResponse.getPageNumber()))
                                                return;
                                            if (!assertEquals(Long.valueOf(2), insurancesListResponse.getPageCount()))
                                                return;
                                        ...
                                        case 1:
                                            ...
                                            flags.add("contractTermination");
                                            if (!assertEquals(Long.valueOf(1), insurancesListResponse.getPageNumber()))
                                                return;
                                            if (!assertEquals(Long.valueOf(2), insurancesListResponse.getPageCount()))
                                                return;
                                            if (!assertEquals(Long.valueOf(1), insurancesListResponse.getPageSize()))
                                            ...
                                            if (!assertEquals(1, insurancesListResponse.getInsurances().size()))
                                                return;
                                            insurance = insurancesListResponse.getInsurances().get(0);

                                            if (!assertEquals("9B070F9C66A91D55A5D4E31F47B147444E651D36", insurance.getId()))
                                                return;
                                            if (!assertEquals(InsuranceType.LIFE, insurance.getType()))
                                                return;
                                            if (!assertEquals("267", insurance.getProduct()))
                                                return;
                                            if (!assertEquals("Pojištění FLEXI JUNIOR", insurance.getProductI18N()))
                                                return;
                                            if (!assertEquals("Hana Bielčíková", insurance.getInsurancePolicyHolder()))
                                                return;
                                            if (!assertEquals("5530446061", insurance.getPolicyNumber()))
                                                return;

                                            life = insurance.getLife();
                                            if (!assertEquals(Long.valueOf(40000), life.getPremium().getValue()))
                                                return;
                                            if (!assertEquals(Integer.valueOf(2), life.getPremium().getPrecision()))
                                                return;
                                            if (!assertEquals("CZK", life.getPremium().getCurrency()))
                                                return;
                                            if (!assertEquals(TimeUtils.getPlainDate("2033-01-31"), life.getContractEndDate()))
                                                return;
                                            if (!assertEquals(TimeUtils.getPlainDate("2011-02-01"), life.getContractStartDate()))
                                                return;
                                            if (!assertEquals(Long.valueOf(0), life.getInsuredAmount().getValue()))
                                                return;
                                            if (!assertEquals(Integer.valueOf(2), life.getInsuredAmount().getPrecision()))
                                            ...
                                            if (!assertEquals(Interval.MONTHLY, life.getPremiumPaymentInterval()))
                                                return;
                                            if (!assertEquals(TimeUtils.getPlainDate("2011-02-15"), life.getLastPremiumDate()))
                                                return;
                                            if (!assertEquals(Long.valueOf(40000), life.getLastPremiumPaid().getValue()))
                                                return;
                                            if (!assertEquals(Integer.valueOf(2), life.getLastPremiumPaid().getPrecision()))
                                                return;
                                            if (!assertEquals("CZK", life.getLastPremiumPaid().getCurrency()))
    ...
    public String getName() {
        return "JInsurancesListPaginationTest";
    }
