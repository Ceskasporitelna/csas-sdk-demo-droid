 ...
 * @since 09/06/16.
 */
public class JAccountsWithIdReservationsListTest extends TestCase {
    ...
    public void run(TestCallback callback) {
        ...
        ((JudgeRestService) mJudgeClient.getService()).nextCase(X_JUDGE_CASE, mXJudgeSessionHeader, new Callback<Response>() {
            ...
            public void success(Response response, Response response2) {
                ...
                mNetbankingJudgeClient.getAccountsResource().withId(accountId).getReservationsResource().list(null,
                        new CallbackWebApi<ReservationsListResponse>() {
                            @Override
                            public void success(ReservationsListResponse reservationsListResponse) {

                                if (!assertEquals(Long.valueOf(0), reservationsListResponse.getPageNumber()))
                                    return;
                                ...
                                if (!assertEquals(Long.valueOf(1), reservationsListResponse.getPageSize()))
                                    return;
                                if (!assertEquals(Long.valueOf(1), reservationsListResponse.getNextPage()))
                                    return;
                                ...
                                if (!assertEquals(ReservationStatus.RESERVED, reservation.getStatus()))
                                    return;
                                if (!assertEquals(TimeUtils.getISO8601Date("2015-09-18T21:43:53Z"), reservation.getCreationDate()))
                                    return;
                                if (!assertEquals(TimeUtils.getISO8601Date("2015-09-25T21:43:53Z"), reservation.getExpirationDate()))
                                    return;
                                if (!assertEquals("Pizzeria Grosseto", reservation.getMerchantName()))
                                    return;
                                if (!assertEquals("Antala Staška 32, Praha", reservation.getCzMerchantAddress()))
                                    return;
                                if (!assertEquals("Platba kartou", reservation.getDescription()))
                                    return;

                                if (!assertEquals(Long.valueOf(45270), reservation.getAmount().getValue()))
                                    return;
    ...
    public String getName() {
        return "JAccountsWithIdReservationsListTest";
    }
