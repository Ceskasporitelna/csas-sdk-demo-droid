 ...
 * @since 09/06/16.
 */
public class JPaymentsListPage0Test extends TestCase {
                        ...
                        mNetbankingJudgeClient.getOrdersResource().getPaymentsResource().list(parameters, new CallbackWebApi<PaymentsListResponse>() {
                                    ...
                                    public void success(PaymentsListResponse paymentsListResponse) {
                                        ...
                                        if (!assertEquals(Long.valueOf(1), paymentsListResponse.getPageSize()))
                                            return;
                                        if (!assertEquals(Long.valueOf(1), paymentsListResponse.getNextPage()))
                                            return;
                                        ...
                                        if (!assertNull(payment.getAdditionalInfo().getText4x35(), "Payment.additionalInfo.text4x35"))
                                            return;
                                        if (!assertEquals(Long.valueOf(110), amount.getValue()))
                                            return;
                                        ...
                                        if (!assertEquals(ChannelId.NET_BANKING, payment.getChannelId()))
                                            return;
                                        if (!assertEquals(TimeUtils.getISO8601Date("2016-03-21T00:00:00+01:00"), payment.getCzOrderingDate()))
                                            return;
                                        ...
                                        if (!assertEquals("2328489013", receiver.getNumber()))
                                            return;
                                        if (!assertEquals("0800", receiver.getBankCode()))
                                            return;
                                        if (!assertEquals("CZ", receiver.getCountryCode()))
                                            return;
                                        if (!assertEquals("CZ5908000000002328489013", receiver.getCzIban()))
                                            return;
                                        ...
                                        if (!assertEquals(SigningState.OPEN, signingObject.getSigningState()))
                                            return;
                                        if (!assertEquals("853eec470df3a40cb58f3504b956285cf59c3ca9c835d277c6706e7cd2b15e35", signingObject.getSignId()))
                                            return;
                                        ...
                                        if (!assertEquals(Boolean.valueOf(true), payment.getStateOk()))
                                            return;
                                        if (!assertEquals(TimeUtils.getPlainDate("2016-03-23"), payment.getTransferDate()))
                                            return;
